...
type UseHomeNavigationActionsParams = {
  ...
  getTabPathForCurrentScope: (tab: Tab) => string;
};
...
export function useHomeNavigationActions({
  ...
  const handleTabChange = useCallback(
    (nextTab: Tab) => {
      ...
      const destination =
        nextTab === "settings" ? getSettingsSectionPath(settingsSection) : getTabPathForCurrentScope(nextTab);
      void navigate({ to: destination });
    },
    [getTabPathForCurrentScope, navigate, setMobileSidebarOpen, settingsSection],
  );
