...
BEGIN
    -- SECURITY: bind caller identity to the claimed manager id.
    IF auth.uid() IS NULL OR auth.uid() <> current_manager_id THEN
        RAISE EXCEPTION 'Access denied: caller identity does not match current_manager_id.';
    END IF;
    ...
    IF matched_invite.engineer_id = current_manager_id THEN
        RETURN jsonb_build_object(
            'success', false,
            'error_code', 'INVALID_OR_EXPIRED',
            'message', 'Accounts cannot self-report.'
        );
    END IF;
    ...
    IF requested_relation = 'direct_manager'::management_relationship_role THEN
        ...
        SET status = 'archived'::management_transition_status,
            ends_at = COALESCE(ends_at, now())
        WHERE engineer_id = current_manager_id
          AND manager_id = matched_invite.engineer_id
          AND relation_type = 'direct_manager'::management_relationship_role
          AND status IN ('active'::management_transition_status, 'in_handover'::management_transition_status);
    END IF;
...
DROP POLICY IF EXISTS "Users can view workspace pinned resources" ON public.pinned_resources;

CREATE POLICY "Users can view workspace pinned resources"
    ON public.pinned_resources FOR SELECT
    TO authenticated
    USING (
        is_engineer_self(workspace_id)
        OR can_manage_engineer(workspace_id)
        OR can_skip_level(workspace_id)
    );

DROP POLICY IF EXISTS "Authenticated users can pin resources" ON public.pinned_resources;
...
CREATE POLICY "Allow authenticated users to read audit entries involving them"
    ...
    USING (
        ...
        OR EXISTS (
            SELECT 1
            FROM public.reporting_relationships rr
            WHERE rr.manager_id = auth.uid()
              AND rr.engineer_id = access_audit_log.actor_id
